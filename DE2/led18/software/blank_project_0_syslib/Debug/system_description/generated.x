 ...
 *
 * Generated: 2017-03-12 16:09:40.558
 *
 */
...
MEMORY
{
    ...
    program : ORIGIN = 0x00002020, LENGTH = 4064
    data_UNUSED : ORIGIN = 0x00003000, LENGTH = 32
    data : ORIGIN = 0x00003020, LENGTH = 4064
}

    /* Define symbols for each memory base-address */
 __alt_mem_program = 0x00002000 ;
 __alt_mem_data = 0x00003000 ;
...
SECTIONS
{
    ...
    .text :
    {
        ...
        *(.dynamic)
        ...
        KEEP (*(.jcr))
        . = ALIGN(32 / 8);
    } >  program =0x3a880100 /* NOP on Nios2 (big endian) */
   ...
   PROVIDE (__flash_exceptions_start = LOADADDR(.exceptions));
    ...
    .program LOADADDR (.rwdata) + SIZEOF (.rwdata): AT (LOADADDR (.rwdata) + SIZEOF (.rwdata))
    {
        PROVIDE (_alt_partition_program_start = ABSOLUTE(.));
        *(.program .program.*)
        . = ALIGN(32 / 8);
        PROVIDE (_alt_partition_program_end = ABSOLUTE(.));
    } > program

    PROVIDE (_alt_partition_program_load_addr = LOADADDR(.program));

    .data : AT (LOADADDR (.program) + SIZEOF (.program))
    {
